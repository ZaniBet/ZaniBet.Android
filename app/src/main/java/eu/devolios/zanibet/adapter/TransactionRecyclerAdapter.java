...
public class TransactionRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>  {
    ...
    public TransactionRecyclerAdapter(Context context, ArrayList<Transaction> transactionList){
        mContext = context;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType){
            case EMPTY_TYPE:
                ...
                return new TransactionRecyclerAdapter.EmptyViewHolder(inflatedView);
            case HEADER_TYPE:
                inflatedView = LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.transaction_header_item, parent, false);
                return new TransactionRecyclerAdapter.HeaderViewHolder(inflatedView);
            case TRANSACTION_TYPE:
                inflatedView = LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.transaction_item, parent, false);
                return new TransactionRecyclerAdapter.TransactionViewHolder(inflatedView);
        }
        return null;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
       if (holder instanceof TransactionRecyclerAdapter.TransactionViewHolder) {
            Transaction transaction = mTransactionList.get(position-1);
            ((TransactionRecyclerAdapter.TransactionViewHolder) holder).bindTransaction(mContext, transaction);
        } else if (holder instanceof TransactionRecyclerAdapter.EmptyViewHolder) {
            ((TransactionRecyclerAdapter.EmptyViewHolder) holder).bindEmpty(mContext);
        }
    }
